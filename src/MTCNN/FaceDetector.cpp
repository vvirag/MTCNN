...
namespace mtcnn
{
	FaceDetector::FaceDetector(const std::string& model_dir, const MODEL_VERSION model_version)
	{
	...
	#endif
		/* load threecaffe::Networks */
		//p
		P_Net.reset( new caffe::Net<float> (model_dir + "/det1.prototxt", caffe::Phase::TEST) );
		P_Net->CopyTrainedLayersFrom(model_dir + "/det1.caffemodel");
		//R
		R_Net.reset( new caffe::Net<float> (model_dir + "/det2.prototxt", caffe::Phase::TEST) );
		R_Net->CopyTrainedLayersFrom(model_dir + "/det2.caffemodel");
		//O
		O_Net.reset( new caffe::Net<float> (model_dir + "/det3.prototxt", caffe::Phase::TEST) );
		O_Net->CopyTrainedLayersFrom(model_dir + "/det3.caffemodel");
		//L
		if( model_version == MODEL_V2)
		{
			L_Net.reset( new caffe::Net<float> (model_dir + "/det4.prototxt", caffe::Phase::TEST) );
	...
	void FaceDetector::wrapInputLayer(boost::shared_ptr<caffe::Net<float> > net, std::vector< cv::Mat >* input_channels)
	{
	...
	void FaceDetector::pyrDown(const std::vector<cv::Mat>& img_channels, float scale, std::vector< cv::Mat >* input_channels)
	{
	...
	void FaceDetector::buildInputChannels(const std::vector< cv::Mat >& img_channels, const std::vector<BoundingBox>& boxes,
	...
	void FaceDetector::generateBoundingBox(const std::vector<float>& boxRegs, const std::vector<int>& box_shape,
	...
	void FaceDetector::filteroutBoundingBox(const std::vector< FaceDetector::BoundingBox >& boxes,
											...
											float threshold, std::vector< FaceDetector::BoundingBox >& filterOutBoxes)
	{
		filterOutBoxes.clear();
		assert(box_shape.size() == cls_shape.size());
	...
	//void FaceDetector::pad(vector< FaceDetector::BoundingBox >& boxes, int imgW, int imgH)
	//{
	//
	//}
	...
	void FaceDetector::nms_cpu(std::vector<BoundingBox>& boxes, float threshold, NMS_TYPE type, std::vector<BoundingBox>& filterOutBoxes)
	{
		filterOutBoxes.clear();
		if(boxes.size() == 0)
			return;
	 ...
	 */
	std::vector< FaceDetector::BoundingBox > FaceDetector::Detect(const cv::Mat& img, const COLOR_ORDER color_order, const IMAGE_DIRECTION orient, int min_size, float P_thres, float R_thres, float O_thres, bool is_fast_resize, float scale_factor)
	{
		/*change image format*/
		cv::Mat sample;
		...
		for(int i = 0; i < all_scales.size(); i ++)
		{
			...
			//Pcaffe::Net forward operation
			const std::vector<caffe::Blob<float>*> out = P_Net->Forward();
			/* copy the output layer to a std::vector*/
			caffe::Blob<float>* output_layer0 = out[0];
			...
			caffe::Blob<float>* output_layer1 = out[1];
			...
			//std::vector<BoundingBox> filterOutRegs;
		...
		if(totalBoxes.size() > 0)
		{
			...
			caffe::Blob<float>* input_layer = R_Net->input_blobs()[0];
			...
			//R_Net forward
			R_Net->Forward();
			/*copy output layer to std::vector*/
			caffe::Blob<float>* output_layer0 = R_Net->output_blobs()[0];
			std::vector<int> box_shape = output_layer0->shape();
			...
			caffe::Blob<float>* output_layer1 = R_Net->output_blobs()[1];
			std::vector<int> cls_shape = output_layer1->shape();
			...
			std::vector<BoundingBox> filterOutBoxes;
			filteroutBoundingBox(totalBoxes, regs, box_shape, cls, cls_shape, std::vector<float>(), std::vector<int>(), R_thres, filterOutBoxes);
		...
		{
			std::vector<cv::Mat> n_channels;
			caffe::Blob<float>* input_layer = O_Net->input_blobs()[0];
			...
			O_Net->Forward();
			/*copy output layer to std::vector*/
			caffe::Blob<float>* output_layer0 = O_Net->output_blobs()[0];
			std::vector<int> box_shape = output_layer0->shape();
			int output_size = box_shape[0] * box_shape[1];
			const float* begin0 = output_layer0->cpu_data();
			const float* end0 = output_size + begin0;
			...
			caffe::Blob<float>* output_layer1 = O_Net->output_blobs()[1];
			...
			caffe::Blob<float>* output_layer2 = O_Net->output_blobs()[2];
		...
		if(totalBoxes.size() > 0 && model_version == MODEL_V2)
		{
			...
			std::vector<cv::Mat> n_channels;
			caffe::Blob<float>* input_layer = L_Net->input_blobs()[0];
			...
			for(int k = 0; k < 5; k++)
			{
				//copy output layer to
				caffe::Blob<float>* output_layer = L_Net->output_blobs()[k];
